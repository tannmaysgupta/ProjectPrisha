...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
                ...
                case 'select':
                    ...
                    $('input_' + data).select('option').each(function (option, ind) {
                        ...
                        if (option && option.selected) {
                            var current = option.readAttribute('data-calcvalue') && calc.hasOwnProperty('insertAsText') && (calc.insertAsText == false || calc.insertAsText === '0' ) ? option.readAttribute('data-calcvalue') : optionValue(option);
                            if(numeric) {
    ...
    countTotal: function (prices) {
        ...
        if (JotForm.discounts.total) {
            ...
            if ($$('.form-payment-total').length > 0) {
    ...
    applyCoupon: function (discount) {
        ...
        if (!discount.paymentType || (discount.paymentType && discount.paymentType === "product")) {
            ...
            if (discount.apply === "product") {
                ...
                if ($$('.form-payment-total b').length > 0) {
                    ...
                    JotForm.discounts.container = new Element('span').insert(discountHTML + '<br><br>').setStyle({fontSize: '10px'});
                }
            } else {
                JotForm.discounts = {
                    shipping: true,
